...
- **Go 1.26.x** (see `go.mod`). Pre-installed in the VM.
- **Yarn 4.15.0** via corepack (see `packageManager` in `package.json`). Run `corepack enable` if `yarn` is not found.
- **GCC** required for CGo/SQLite compilation of the backend.
- **PATH gotcha**: a pre-installed `/exec-daemon/node` (Node v22) sits ahead of nvm in `PATH`, so `node` resolves to v22 even after `nvm use`. To run frontend commands with the `.nvmrc` version, prepend the nvm bin dir in the shell: `export PATH="$HOME/.nvm/versions/node/$(cat .nvmrc)/bin:$PATH"`.
